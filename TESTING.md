...
I noticed that if I select to remove a current image, and then select a new image, and then click **Update**, the form is unvalid and throws an error toast message. I decided to add some simple jQuery to hide the **Select image** button and text when you select the checkbox to remove the current image, and the other way around, when you select a new image first, the checkbox for removal will be hidden. This should help the user understand that they can only do one of those things before they click **Update**.
