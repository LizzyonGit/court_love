...
- On the **My profile** page, I got an error about a duplicate id attribute, caused by the django widget handling the image upload. [Slack](https://code-institute-room.slack.com/archives/C026VTHQDNY/p1683142277576629) helped me to understand the issue, so I replaced the id "new-image" I set with a class of the same name, and updated the jQuery for this. This worked and the error disappeared.

  Another issue on the same page was that I had set a placeholder for a select element (**Level**), I found a similar error on [Slack](https://code-institute-room.slack.com/archives/C026VTHQDNY/p1714631167308539). I decided to remove the code about placeholders (this was copied and adapted from other fields) because it actually only was applicable to the **Phone number** field, which already had a label. So the error also disappeared.
...
- Best practices warns about third party cookies from Stripe, and about issues logged to the Issue panel, also related to Stripe, so I cannot change that. Actually, in incognito mode, this is gone.
...
- Home page
  ...
  ![Home desk](docs/testing/lighthouse/desk-home.png)


  Mobile:


  ![Home mobile](docs/testing/lighthouse/mobile-home.png)
...
- All lessons page
  ...
  ![All lessons desk](docs/testing/lighthouse/desk-lessons-final.png)
...
|find necessary information about lessons so that I can book lessons that suit my needs.|The lesson cards with date, time, duration, description, place, level, capacity and price, and the addresses on top of the **All lessons** page, and in the footer.|
...
I noticed that if I select to remove a current image, and then select a new image, and then click **Update**, the form is unvalid and throws an error toast message. I decided to add some simple jQuery to hide the **Select image** button and text when you select the checkbox to remove the current image, and the other way around, when you select a new image first, the checkbox for removal will be hidden. This should help the user understand that they can only do one of those things before they click **Update**. This however resulted in the text and checkbox div being too long to be on one line on small screens, so I decided to shorten this text to *Remove current image after Update*.
...
|Carousel|Starts rolling when you initiate yourself|On page load, I checked if it would run, then I clicked the carousel arrows|The carousel does not run on page load, it starts running after clicking one of the arrows|Pass|
|Footer|
